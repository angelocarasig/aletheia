...
extension ModelBundle {
    ...
    private func rank(against url: URL, log: (String) -> Void) throws {
        ...
        var exact = 0, wrongOrder = 0, drifted = 0
        var worst: Float = 0
        let started = Date()
        ...
        for test in cases {
            ...
            guard mine == want else {
                ...
                if wrongOrder <= 3 {
                    ...
                    log("  RANK MISMATCH seed \(test.seedRow) \(test.seedTitle.prefix(30)) "
                        + "overlap \(overlap)/\(want.count), first differs at \(first)")
                }
                continue
            }
            let delta = zip(got.ranked, test.results).map { abs($0.score - $1.score) }.max() ?? 0
            worst = max(worst, delta)
            if delta < 1e-4 { exact += 1 } else { drifted += 1 }
        }

        let per = Date().timeIntervalSince(started) / Double(max(cases.count, 1))
        log("ranking - \(exact)/\(cases.count) exact, \(wrongOrder) wrong order, "
    ...
    static func probe(_ recommender: Recommender) async {
        ...
        log("recommender \(descriptor.slug) - \(descriptor.titleCount) titles, "
        ...
        do {
            ...
            log(String(format: "  resolved \"%@\" to row %d (%d vote) in %.0fms, wTagEff %.2f",
                       matched, row, votes, took * 1000, set.wTagEff))
            for r in set.results.prefix(3) {
                log("    \(r.title) [\(r.catalogId.rawValue)] "
            ...
            let projected = try await recommender.recommend(
                Payload(titles: ["zzzz no such series zzzz"],
            ...
            if case .projected(let encoded, let dropped) = projected.seed {
                log("  projected - \(encoded) columns encoded, \(dropped) tags dropped, "
            ...
            AppLog.shared.log("recommend FAILED - \(error)", level: .error, category: "recommender")
        }
    }
    ...
    static func probe() {
        ...
        do {
            ...
            let first = String(data: titles[Int(titleOff[0])..<Int(titleOff[1])],
                               encoding: .utf8) ?? "?"

            log("model v\(bundle.manifest.formatVersion) mapped in "
            ...
            if let pack = bundle.metadata {
                ...
                expect(Int(synopsis[n]) == (try bundle.blob("meta-synopsis.blob")).count,
                       "synopsis blob length")
                log("metadata v\(pack.metadataVersion) - \(pack.statuses.count) statuses, "
            ...
            var probed = 0, hit = 0, elsewhere = 0, absent = 0, multi = 0
            let started = Date()
            for row in stride(from: 0, to: n, by: step) {
                let title = String(data: titles[Int(titleOff[row])..<Int(titleOff[row + 1])],
                                   encoding: .utf8) ?? ""
                guard !title.isEmpty else { continue }
                probed += 1
                let found = index.candidates(for: title)
                if found.count > 1 { multi += 1 }
                if found.isEmpty { absent += 1 }
                else if found.contains(row) { hit += 1 }
                else { elsewhere += 1 }
            }
            let per = Date().timeIntervalSince(started) / Double(max(probed, 1))
            log("alias lookup - \(probed) titles, "
            ...
            log(String(format: "cold cost - map %.0fms, vocabulary %.0fms, ids scan %.0fms (ascending %@)",
            ...
            for row in stride(from: 0, to: n, by: max(1, n / 300)) {
                let lo = Int(indptr[row]), hi = Int(indptr[row + 1])
                guard hi - lo >= 5 else { continue }
                var columns: [Int] = [], stored: [Int: Float] = [:]
                for i in lo..<hi {
            ...
            if !cosines.isEmpty {
                ...
                log("payload encoder - \(cosines.count) rows re-encoded, "
                    + String(format: "cosine mean %.3f min %.3f max %.3f",
            ...
            expect(vocabulary.hints(in: "Solo Leveling (Manhwa)").contains("manhwa"), "type hints")
            ...
            var withEmbedding = -1, withoutEmbedding = -1
            for row in 0..<n {
                if withEmbedding < 0, hasEmbed[row] == 1, year[row] != Int16(bundle.manifest.yearUnknown) {
                    withEmbedding = row
                }
                if withoutEmbedding < 0, hasEmbed[row] == 0, year[row] != Int16(bundle.manifest.yearUnknown) {
            ...
            if withEmbedding >= 0 && withoutEmbedding >= 0 {
                ...
                log(String(format: "query cost - full %.0fms, without embedding block %.0fms, "
            ...
            if let url = Foundation.Bundle.main.url(forResource: "ranking", withExtension: "json") {
            ...
            if problems.isEmpty {
                ...
                AppLog.shared.log("bundle self-check FAILED - \(problems.joined(separator: ", "))",
            ...
            if case .unavailable = error {
                ...
                AppLog.shared.log("model load FAILED - \(error.detail)",
                                  level: .error, category: "recommender")
            }
        } catch {
            AppLog.shared.log("model load FAILED - \(error)",
                              level: .error, category: "recommender")
        }
    }
}
