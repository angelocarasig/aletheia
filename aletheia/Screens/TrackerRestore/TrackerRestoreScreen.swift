...
struct TrackerRestoreScreen: View {
    let composer: TrackerRestoreComposer
    let onFinish: () -> Void
    ...
    private var FilterPills: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: dimensions.spacing.space8) {
                ForEach(TrackerRestoreComposer.RowFilter.allCases) { filter in
                    FilterPill(filter)
                }
            }
            .padding(.horizontal, dimensions.screenMargin)
    ...
    private func FilterPill(_ filter: TrackerRestoreComposer.RowFilter) -> some View {
        let active = composer.filter == filter
        let count = composer.count(for: filter)

        return Text("\(filter.label) (\(count))")
            .font(.subheadline)
