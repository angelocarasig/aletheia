...
struct DetailsScreen: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            // outside the branch so the skeleton sits on it too - previously
            // rendered against the system background
            Palette.canvas
                .ignoresSafeArea()
        ...
        .sheet(isPresented: $showingSetup) {
            if let composer {
                ...
                DetailsSetup(
                    ...
                    // presented inline rather than via the state-driven sheets above:
                    // a second .sheet on this view will not open over the first one
        ...
        .sheet(isPresented: $showingScanlatorOrder) {
            if let composer {
                ...
                // needs every scanlator including ones currently winning nothing,
                // which the screen's own list excludes
    ...
    private func Loaded(_ composer: DetailsComposer) -> some View {
        ...
        .transition(.opacity)
        // clamped and rounded inside the transform, not the action: the callback
        // only fires when the transformed value changes, so this is what stops
        // it firing for the rest of the chapter list past the blur ramp
        ...
        // safeAreaInset not overlay: this bar is permanent, an overlay would
        // permanently hide the last chapter row
        ...
        // also keyed on live(composer), or the rebuilt-pill case would appear
        // and leave without a transition
    ...
    // rebuilt from the shared unit, not remembered, so the pill reappears after
    // the screen is closed and reopened mid-fetch. an origin that finished while
    // the screen was gone is excluded rather than given an invented result
