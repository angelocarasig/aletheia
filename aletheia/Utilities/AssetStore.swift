...
struct AssetStore: AssetStoring {
    ...
    func store(_ asset: Asset, progress: (@Sendable (Int, Int) -> Void)? = nil) async throws
        -> String
    {
    ...
    private static func existing(_ stem: URL) -> URL? {
        ...
        return entries.first {
            $0.lastPathComponent == name || $0.lastPathComponent.hasPrefix("\(name).")
        }
    }
    ...
    func sweep(_ folder: URL, keeping live: Set<String>) throws -> Int {
        ...
        for shard in shards {
            let entries =
                (try? manager.contentsOfDirectory(at: shard, includingPropertiesForKeys: nil)) ?? []
            ...
            for entry in entries where !live.contains(Constants.Paths.relative(entry)) {
                do {
                    ...
                    log.log(
                        "could not remove \(entry.lastPathComponent) - \(error)", level: .error,
                        category: "assets")
                }
            }
        }
