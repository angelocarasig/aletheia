...
// Progress" and nothing about registration, submission or expiry, and the
// task's own progress is read-only, so an owner cannot hand over one of its own
@MainActor
final class ContinuedTask {
    // nil means the run is over - the only thing this type needs to know
    ...
    // runs on the heartbeat, immediately before the progress it feeds
    ...
    // nonisolated so an owner built off the main actor during bootstrap can
    // hold one - legal since it only assigns stored properties
    nonisolated init(
        identifier: String,
        log: AppLog = .shared,
    ...
    // must complete before launch ends - a launch the system started for the
    // task itself has no screens to do it from
    ...
    func submit(title: String, subtitle: String) {
        ...
        guard task == nil else {
            ...
            do {
                ...
                // a failed submission is not a failed run - the work is already
                // going in the foreground, it just won't survive being backgrounded
    ...
    // attaches rather than starts - the system grants this after the work is
    // already going. a run that finished in the meantime completes it
    // immediately, since there's nothing left to extend
